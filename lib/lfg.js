module.exports = (function() {
    ...
    function set_key_location(session_key, cartographic_pos) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            xhr.open('POST', '/set_key_location', true);
    ...
    class LastFMGlobeViewer {
        constructor(container, key) {
            ...
            this.our_key = key || null;

            this.markers = {};
        }

        add_marker(key, pos, options) {
            options = options || {};
            const useExisting = options.useExisting || false;

            if(useExisting) {
                var existing = this.viewer.entities.getById(key);
        ...
        set_user_session_key(key) {
            this.our_key = key || this.our_key;
        }

        start_user_location_wizard() {
        ...
        enable_location_picking_mode(options) {
            ...
            // Find session key
            var session_key = options.session_key || this.our_key;
            if(!session_key) {
                throw "User's session key must be available. Use" +
                " set_user_session_key(key)";
            }

            // If there isn't a marker already, make one!
            var marker = this.viewer.entities.getById(session_key);
            if(!marker) {
                var marker = this.add_marker(this.our_key, null);
            }
            ...
            show_toolbar_button(tbar, "Set my location!", async (remove) => {
                ...
                try {
                    ...
                    // Record position
                    await set_key_location(session_key, cartographic_pos);
        ...
        async enable_viewer() {
            ...
            for(var i = 0; i < res.length; ++i) {
                ...
                user.marker = this.add_marker(user.user.key, pos, {
                    useExisting: true
                });
            }
        }
